...
* comment.
*  @Service
*  - @Component 의 특수한 형태다. 서비스 계틍에 속하는 Bean 을 명시한다.
*  - 역할 : 비지니스 로직을 처리하는 클래스
* */
...
public class PaymentService {
    ...
    /*
    * comment.
    *  @Autowired : 스프링의 핵심 어노테이션 중 하나이며 객체 간 의존성을 자동으로 주입해주는 역할을 하게 된다.
    *  ex) controller -> service -> dao
    *  - Service 에서 PaymentInterface 를 의존하려고 할  때 Error 가 발생하는 이유는
    *  - PaymentInterface 를 상속받은 구현체가 2개 이기 때문에 어떤 Bean 을 주입해야 할 지 모르기 때문에 발생하는 Error 이다.
    * */
    ...
    public boolean processPayment(String orderId, double amount) {
        System.out.println("결제 비지니스 로직 시작. 주문 ID = " + orderId + ", 금액 = " + amount);
