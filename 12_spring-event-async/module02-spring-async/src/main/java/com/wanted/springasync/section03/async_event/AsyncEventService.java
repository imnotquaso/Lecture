...
public class AsyncEventService {
    ...
    public LectureResponse completeEnrollment(Long enrollmentId) {
        ...
        /*
         * comment.
         *  해당 서비스는 수료 서비스와 의존성을 분리하며, 수강 완룔는 이벤트를 발행 할 것이다.
         *  또한 해당 이벤트는 동기 방식으로 진행하지 않으며, 비동기 방식으로 구성할 것 이다.
         * */
        // G1) notification 과 의존성 맺지 않고 event 발행으로 처리
//        asyncNotificationService.sendCompletionEmail(enrollment);
    ...
    public CompletionSummaryResponse requestCompletionSummary(Long enrollmentId) {
        ...
        * comment.
        *  thenAccept() 의 흐름
        *  1. createSummaryAsync 메서드 호출 시 CompletableFuture 타입의 값을 죽시 받는다.
        *  2. thenAccept( 매개변수 -> 실행 식 ) 으로 나중에 성공하면 실행 될 코드를 작성한다.
        *  3. 현재 메인 요청 스레드는 콜백 실행을 기다리지 않고 메인 흐름을 계속 진행한다.
        *  4. 비동기 작업이 모두 완료되면 Summary 값이 콜백 함수에 전달되고, 그 떄 로그가 출력된다.
        * */

        // 수강 완료 시 수강 완료 요약 정보 생성은 비동기로 진행한다.
        completionSummaryService.createSummaryAsync(enrollmentId).thenAccept(
                // G1) summary 는 미래에 담길 값, 변수 이름, 빈 값
                // G1) 나중에는 log 자리에 실행해야할 코드 작성
                // 비동기 결과 -> 비동기 결과가 도출 되었을 때 실행할 비지니스 로직(비동기 결과)
                // 이런 식으로 활용할 수 있게 된다.
                summary -> log.info(
                        "[section03] CompletableFuture 콜백 발생. summary = {} , thread = {}" , summary, Thread.currentThread().getName()
                )
        );
        ...
        return CompletionSummaryResponse.accepted(
                "메인흐름 완료. 비동기 수강 요약 생성은 백그라운드에서 계속 진행 중",
                start
        );
    }
    ...
    public CompletionSummaryResponse waitrequestCompletionSummary(Long enrollmentId) {
        ...
        /*
        * comment.
        *  join() 은 현재 메인 흐름의 요청 스레드를 멈춰세우고, Future 결과가 채워질 떄까지 기다린다.
        *  즉, @Async 메서드의 결과가 도출될 때까지 기다린다고 생각하면 된다.
        *  // G1) join() 은 예외 처리가 필수가 아님
        *  get() : 예외 처리가 필수적이다. 실무에서는 join() 을 활용하기 보다는
        *  예외처리가 강제적인 get(timeout, unit) 형식으로 작성해서 최대 대기 기간을 설정하여
