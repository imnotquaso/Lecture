...
public class ProblemExJDBC {
    ...
    /*
    * comment.
    *  테스트코드 기반으로 JPA 를 사용하지 않을 때 발생할 수 있는 문제를 예시로 알아보자.
    * */
    ...
    *  메소드 별로 독립적인 실행이 가능하며, 우리가 만든 기능을 테스트 하는 것에 초점을 맞춘다.
    *  또한 접근제한자는 사용하지 않아도 되며, 단 private 는 허용되지 않는다.
    * */

    // DB 연결시 필요한 com 객체 선언
...
//        }
    ...
    *  이렇게 요구가 바뀌게 되면 데이터베이스 DDL 구문 수정해야하며, SQL 구문 수정, Application 코드 수정이 필요하다.
    * */

    /*
    * comment.
    *  3. 패러다임 불일치 문제(객체지향 관점에서 벗어난다.)
    *  - SQL 관점에서 1:N -> 엔티티 간 연관성울 FK 로 가지고 있는다.
    ...
    * */
    ...
    void equalsTwoObject() throws SQLException {
        ...
        *  -> 한 객체에서 수정을 하게 되면 다른 객체는 수정을 알아차릴 수 없다. 그만 하자 쫌,,,
        * */
