...
public class Category {
    ...
    *  자식 == 필드로 가지고 있는 menuList 도 같이 영속성 컨텍스트에서 꺼내와야 한다.
    ...
    *  - 메뉴들도 DB 에서 함께 삭제한다.
    *  - 부모 엔티티와 연관 된 자식 엔티티는 부모가 삭제되면 고아 데이터가 되기 때문에 자동으로 삭제하기 위함이다.
    * */

    @OneToMany(cascade = CascadeType.PERSIST, fetch = FetchType.EAGER) // 하나의 카테고리는 여러 개의 메뉴를 가지고 있음
