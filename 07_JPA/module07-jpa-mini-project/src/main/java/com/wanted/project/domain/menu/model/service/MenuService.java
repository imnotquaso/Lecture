...
public class MenuService {
    ...
    public List<MenuDTO> findMenuByPrice(int menuPrice) {
        ...
        // stream() 은 collection 객체를 반복 가능하게 나열, map() 하여 변환
...
//        menuRepository.save(modelMapper.map(registMenu, Menu.class));
    ...
    public void modifyMenuName(int menuCode, String menuName) {
        ...
        /* 1. setter 사용해서 update -> setter 사용은 지양한다.(추적의 어려움) */
//        foundMenu.setMenuName(menuName);

        /* 2. @Builder 어노테이션을 사용한 업데이트 기능 */
//        foundMenu = foundMenu.toBuilder().menuName(menuName).build();
        // 새로운 인스턴스가 대입 되는 것은 아직 영속성 컨텍스트가
...
//        menuRepository.save(foundMenu);
    ...
    public void deleteMenu(int menuCode) {
        if (!menuRepository.existsById(menuCode)) {
            throw new IllegalArgumentException("해당 메뉴 번호가 존재하지 않습니다: " + menuCode);
        }
