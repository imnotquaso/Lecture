...
/*
 * CourseRepositoryAdapter 는 Domain 계층의 CourseRepository Port 를 JPA 로 구현하는 Adapter 다.
 *
 * 계층별 호출 흐름:
 ...
 * - DB 저장 구조가 바뀌어도 Course Aggregate 의 비즈니스 규칙을 최대한 보호한다.
 *
 * JPA 프록시 관련 주의:
 * - @ManyToOne(fetch = LAZY), @OneToMany 같은 연관관계는 실제 객체 대신 프록시나 지연 로딩 컬렉션일 수 있다.
 * - 트랜잭션이 끝난 뒤 프록시를 건드리면 LazyInitializationException 이 발생할 수 있다.
 * - 그래서 Adapter 안의 트랜잭션 범위에서 필요한 연관 데이터를 초기화하고 순수 Domain Model 로 변환한다.
 * - Controller 나 Application Service 로 JPA Entity 를 그대로 반환하지 않는다.
 */
@Repository
@Transactional
// G1) Domain 의 CourseRepository 인터페이스와 JPA 의 SpringDataCourseRepository 사이를 어댑터가 통역해주는 형식
// G1) service 가 repository 를 불렀으나,  repositoryAdapter 가 해당 클래스를 상속받았으므로 호출됨
public class CourseRepositoryAdapter implements CourseRepository {
    ...
    public Course save(Course course) {
        /*
         * 저장 흐름:
         * 1. 새 Course 면 CourseJpaEntity 를 새로 만든다.
         * 2. 기존 Course 면 영속 상태의 CourseJpaEntity 를 조회한다.
         * 3. Domain Model 의 상태를 JPA Entity 에 반영한다.
         * 4. 저장된 JPA Entity 를 다시 Domain Model 로 변환해 반환한다.
         *
         * Application Service 는 이 내부 과정을 모르고 CourseRepository Port 만 사용한다.
         */
        // G1) 1. Course(도메인) -> CourseJpaEntity(JPA) 변환
        ...
        // G1) CourseJpaEntity(JPA) -> Course(도메인) 다시 변환해서 반환
    ...
    // G1) Optional 예외 처리를 QueryService 에서 처리
    ...
    private Course toDomain(CourseJpaEntity entity) {
        /*
         * JPA Entity 를 순수 Domain Model 로 변환한다.
         *
         * 왜 Hibernate.initialize()를 호출하는가?
         * - entity.getSections()는 JPA 가 관리하는 지연 로딩 컬렉션일 수 있다.
         * - section.getModules()도 지연 로딩 컬렉션일 수 있다.
         * - 이 Adapter 를 벗어난 뒤에는 트랜잭션/영속성 컨텍스트가 닫힐 수 있다.
         * - 닫힌 뒤 프록시를 접근하면 LazyInitializationException 이 발생한다.
         *
