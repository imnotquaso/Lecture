...
/*
 * CourseSectionJpaEntity 는 course_sections 테이블과 매핑되는 JPA 저장 모델이다.
 *
 * JPA 프록시 관점:
 * - course 는 @ManyToOne(fetch = LAZY)이므로 실제 CourseJpaEntity 가 아니라 프록시일 수 있다.
 * - 프록시는 필요한 순간 DB 에서 값을 가져오기 위해 Hibernate 가 만든 대리 객체다.
 * - 영속성 컨텍스트가 닫힌 뒤 프록시를 접근하면 LazyInitializationException 이 발생할 수 있다.
 *
 * 그래서 이 Entity 는 Infrastructure 계층 안에서만 사용하고,
 * CourseRepositoryAdapter 가 CourseSection Domain Model 로 변환한 뒤 바깥 계층에 넘긴다.
 *
 * JPA Entity 메서드의 기준:
...
public class CourseSectionJpaEntity {
    ...
    /*
     * LAZY 를 사용하는 이유:
     * - section 을 조회할 때 항상 course 전체가 필요한 것은 아니다.
     ...
     * 모듈 추가 가능 여부나 순서 중복 검증은 Domain Model 인 CourseSection.addModule()에서 처리한다.
     */
