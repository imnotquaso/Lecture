...
import com.wanted.cleanarchitecture.global.domain.common.event.CoursePublishedEvent;
...
/*
 * Course 는 catalog Bounded Context 의 Aggregate Root 다.
 *
 * 계층별 호출 흐름:
 * 1. CourseController 가 HTTP 요청을 Command 로 변환한다.
 * 2. CourseCommandService 가 CourseRepository 로 Course 를 조회한다.
 * 3. CourseCommandService 가 Course.publish() 같은 도메인 행위를 호출한다.
 * 4. Course 는 자기 상태를 기준으로 비즈니스 규칙을 검증하고 상태를 바꾼다.
 * 5. Course 는 중요한 결과가 발생하면 DomainEvent 를 내부에 기록한다.
 * 6. CourseCommandService 가 Course 를 저장한 뒤 pullDomainEvents()로 이벤트를 꺼내 발행한다.
 *
 * 중요한 점:
 * - Course 는 HTTP, JPA, Spring Event 발행 방식을 모른다.
 * - Course 는 "강의가 공개 가능한가?" 같은 순수 도메인 규칙만 책임진다.
 * - CourseSection 과 ContentModule 은 Course 를 통해서만 변경되어야 Aggregate 규칙이 보호된다.
 *
 * 도메인 클래스의 메서드 기준:
 * - publish(), addSection(), addModule()처럼 업무 행위를 표현한다.
 * - ensureDraft(), validatePublishable()처럼 비즈니스 규칙을 검증한다.
 * - CoursePublishedEvent 처럼 업무적으로 의미 있는 결과를 기록한다.
 *
 * JPA Entity 메서드와의 차이:
 * - CourseJpaEntity.changeStatus()는 DB 컬럼 값을 맞추기 위한 저장 모델 메서드다.
 * - Course.publish()는 공개 규칙 검증, 상태 변경, 이벤트 기록을 포함하는 도메인 행위다.
 * - 따라서 Application Service 는 가능하면 CourseJpaEntity 가 아니라 Course 의 도메인 메서드를 호출해야 한다.
 */

// G1) @Entity <- 없음(순수 자바 객체)
public class Course {
    ...
    /*
     * domainEvents 는 Aggregate 안에서 발생한 DomainEvent 를 잠시 보관하는 버퍼다.
     *
     * 왜 바로 발행하지 않는가?
     * - Domain 계층은 Spring 의 ApplicationEventPublisher 를 몰라야 한다.
     * - 저장이 실패했는데 이벤트만 발행되면 잘못된 후속 처리가 실행될 수 있다.
     * - 따라서 Aggregate 는 기록만 하고, 저장/트랜잭션 경계를 아는 Application Service 가 발행한다.
     */
    private final List<DomainEvent> domainEvents = new ArrayList<>();

    // G1) 비지니스 규칙 검증을 여기서 진행함
    ...
    // G1) 새로 만들 때 -> id = null, status = DRAFT 고정
     ...
     */
    /*
     * occurredAt을 Instant 로 받는 이유:
     * - 강의 공개라는 도메인 결과가 발생한 정확한 시점을 기록하기 위해서다.
     * - Course 내부에서 Instant.now()를 직접 호출하지 않으면 테스트에서 시간을 고정할 수 있다.
     * - 현재 시간을 구하는 책임은 Application Service 의 Clock 이 담당하고,
     *   Course 는 전달받은 시각을 Domain Event 에 기록하기만 한다.
     */
    public void publish(Instant occurredAt) {
        ...
        // G1) Event 를 바로 발행 안하고 해당 클래스에 쌓아둠
        //  Domain 이 Spring 의 ApplicationEventPublisher 를 알아선 안됨.
        //  저장 실패시, 이벤트를 만나면 안되기 때문에 Service 가 저장완료 후 꺼내어 발행
    }
    ...
    /*
     * pullDomainEvents()는 Aggregate 가 기록한 이벤트를 Application Service 로 넘겨주는 출구다.
     *
     * 이름이 pull 인 이유:
     * - Application Service 가 저장 후 필요한 시점에 이벤트를 "꺼내 간다".
     * - Aggregate 가 외부 발행기를 직접 호출하지 않는다.
     *
     * 왜 꺼낸 뒤 비우는가?
     * - 같은 DomainEvent 가 여러 번 발행되면 알림, 통계, 후속 처리도 중복 실행될 수 있다.
     * - 따라서 한 번 꺼낸 이벤트는 내부 목록에서 제거한다.
     *
     * 흐름:
     * Course.publish() -> domainEvents 에 CoursePublishedEvent 기록
