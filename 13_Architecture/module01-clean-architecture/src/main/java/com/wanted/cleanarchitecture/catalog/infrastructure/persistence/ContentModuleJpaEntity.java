...
/*
 * ContentModuleJpaEntity 는 content_modules 테이블과 매핑되는 JPA 저장 모델이다.
 *
 * JPA 프록시 관점:
 * - section 은 @ManyToOne(fetch = LAZY)이므로 ContentModuleJpaEntity 를 읽는 순간
 *   CourseSectionJpaEntity 전체가 즉시 로딩되지 않을 수 있다.
 * - JPA 는 section 자리에 프록시 객체를 넣어두고, 실제 접근 시점에 DB 조회를 시도한다.
 *
 * Clean Architecture 관점:
 * - 이 프록시 동작은 Infrastructure 세부사항이다.
 * - Domain Model 인 ContentModule 은 프록시, 영속성 컨텍스트, Lazy Loading 을 몰라야 한다.
 * - 변환 책임은 CourseRepositoryAdapter 가 가진다.
 *
 * JPA Entity 메서드의 기준:
...
public class ContentModuleJpaEntity {
     ...
     * "어떤 섹션에 모듈을 추가할 수 있는가"는 Domain Model 에서 판단한다.
     */
