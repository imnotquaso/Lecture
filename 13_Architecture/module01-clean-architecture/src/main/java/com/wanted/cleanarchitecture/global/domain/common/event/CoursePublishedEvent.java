...
/*
 * CoursePublishedEvent 는 "강의가 공개되었다"는 도메인 결과다.
 *
 * Command 와의 차이:
 ...
 *
 * Instant 를 사용하는 이유:
 * - Domain Event 는 "언제 발생했는가"를 남겨야 한다.
 * - LocalDateTime 은 시간대 정보가 없어 서버/지역이 달라지면 해석이 달라질 수 있다.
 * - Instant 는 UTC 기준의 한 시점을 표현하므로 로그, 이벤트, 통계, 외부 시스템 연동에 적합하다.
 *
 * 수업용 정리:
 * Domain Event 에는 후속 처리에 필요한 최소 정보만 담는다.
