...
/*
 * CourseJpaEntity 는 courses 테이블과 매핑되는 JPA 저장 모델이다.
 *
 * JPA Entity 메서드의 기준:
 * - DB에 저장할 상태를 변경하거나 연관관계를 맞춘다.
 * - JPA 의 cascade, orphanRemoval, 양방향 연관관계 같은 저장 기술을 다룬다.
 ...
 *
 * 수업용 정리:
 * JPA Entity 의 메서드는 "DB에 어떻게 저장할지"를 돕고,
 * Domain Model 의 메서드는 "업무적으로 무엇을 할지"를 표현한다.
 */
@Entity // G1) JPA 가 이 클래스를 DB 테이블과 매핑
@Table(name = "courses") // G1) Courses 테이블
...
public class CourseJpaEntity {
    ...
    /*
     * CourseJpaEntity 가 Aggregate Root 저장 모델이므로 하위 section 을 cascade 로 함께 저장한다.
     *
     * orphanRemoval = true:
     * - Course 에서 제거된 section 은 DB 에서도 제거되도록 한다.
     *
     * 주의:
     * - 이 설정은 저장 기술의 세부사항이다.
     * - Course Domain Model 은 cascade 나 orphanRemoval 을 몰라야 한다.
     */
    // G1) cascade : Course 저장 시 Section 도 같이 저장
    //  orphanRemoval : Course 에서 Section 이 빠지면 DB 에서도 같이 삭제
     ...
     * 공개 가능 여부 검증은 CourseJpaEntity 가 아니라 Course Domain Model 이 담당한다.
     */
    ...
    /*
     * replaceSections()는 Course Aggregate 의 섹션 상태를 JPA 컬렉션에 동기화하는 저장용 메서드다.
     *
     * section.assignCourse(this)를 호출하는 이유:
     * - JPA 양방향 연관관계에서 자식 Entity 가 부모 FK를 알도록 맞춰야 한다.
