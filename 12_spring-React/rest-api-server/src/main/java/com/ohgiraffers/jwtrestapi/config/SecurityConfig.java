...
public class SecurityConfig {
    ...
    /* 목차. 1. 암호화 처리를 위한 PasswordEncoder 를 빈으로 설정(빈을 등록 시 메소드 이름 오타 없도록 주의!) */
    @Bean
    ...
    /* 목차. 3. HTTP 요청에 대한 권한별 설정 (세션 인증 -> 토큰 인증으로 인해 바뀐 부분 존재) */
    @Bean
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        ...
        http
                ...
                .authorizeHttpRequests(auth -> {
                    /* 설명.
                     *  CORS 를 위해 preflight 요청 처리용 options 요청 허용.
                     *  ====================================================================================================
                     *  preflight request 란?
                     *  요청 할 url 이 외부 도메인일 경우 웹 브라우저에서 자체 실행되며 options 메소드로 사전 요청을 보내게 된다.
                    ...
                    // 어떤 요청이든 허용 -> Security 를 활용한 로그인이 모두 완성되지 않았을 때 사용할 것
//				auth.anyRequest().permitAll();
    ...
    /* 목차. 4. CORS 설정용 Bean(허용 할 origin 과 httpMethod 종류와 header 값) */
    @Bean
    // CORS 커스텀 설정 객체
    CorsConfigurationSource corsConfigurationSource() {
        ...
        configuration.setAllowedHeaders(
                Arrays.asList(
                        ...
                        "Authorization",
                        //XMLHttpRequest 로 요청이 이루어졌는지를 나타내기 위해 사용
                        "X-Requested-With"
                )

                /*
                 * HTTP Req
                 * -> header (body 에 대한 상세 정보, 인증/인가 관련 데이터), body(게시글 등 보내는 내용)
                 * */
...
/*@CrossOrigin("") : 각 controller 에서 작성 금지, 해당 어노테이션은 여러 개의 Http 를 사용할 때만 이용
* 전역 차원에서 HTTP 요청 받기*/