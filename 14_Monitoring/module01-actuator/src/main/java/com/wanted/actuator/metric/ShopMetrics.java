...
public class ShopMetrics {
    ...
    // MeterRegistry 는 Micrometer의 핵심 진입점이다.
    // Counter, Timer, DistributionSummary 같은 Meter 를 등록하고 기록하는 저장소 역할을 한다.
    // Spring Boot Actuator 가 있으면 기본적으로 SimpleMeterRegistry 가 만들어지고,
    // Prometheus 의존성을 추가하면 PrometheusMeterRegistry 가 함께 사용된다.
    private final MeterRegistry meterRegistry;

    // Counter 는 감소하지 않는 누적 값을 표현한다.
    // "주문이 성공한 횟수", "인기 상품 API 가 호출된 횟수"처럼 사건 발생 수에 적합하다.
    private final Counter createdOrderCounter;
    private final Counter popularProductRequestCounter;

    // DistributionSummary 는 시간(duration)이 아닌 값의 분포를 기록한다.
    // 주문당 상품 종류 수, 주문 금액처럼 "값이 얼마나 큰가"를 관찰할 때 사용한다.
    private final DistributionSummary orderItemSummary;
    private final DistributionSummary orderAmountSummary;

    // Timer 는 실행 시간과 호출 수를 함께 기록한다.
    // Actuator 에서 count, totalTime, max 같은 값을 확인할 수 있다.
    ...
    public ShopMetrics(MeterRegistry meterRegistry) {
        ...
        // 단순 합계뿐 아니라 count, total, max 를 확인할 수 있다.
    ...
    // Timer.Sample 은 작업 시작 시점의 시간을 기억한다.
    // 실제 Timer 족 는 작업이 끝난 뒤 sample.stop(timer) 를 호출할 때 결정된다.
    // 이렇게 하면 try-finally 로 감싸서 성공/실패 여부와 관계없이 시간을 기록할 수 있다.
    public Timer.Sample startTimer() {
        return Timer.start(meterRegistry);
    }

    // 주문 성공 메트릭은 단순한 메서드 실행 횟수가 아니라 "실제로 주문 저장에 성공했다"는 비즈니스 의미를 가진다.
    // 그래서 AOP 로 숨기기보다 서비스에서 성공 지점에 명시적으로 호출하는 것이 수업과 실무 모두에서 이해하기 쉽다.
    ...
    // 전체 API 오류는 recordApiError 에서 따로 기록한다.
    // 이 둘을 구분해야 "주문 도메인 문제"와 "HTTP 사용자 경험 문제"를 분리해서 볼 수 있다.
    public void recordFailedOrder(RuntimeException exception) {
        // 실패 원인은 제한된 태그 값만 사용한다.
        // 예외 메시지를 태그로 넣으면 메시지 종류만큼 시계열이 늘어날 수 있으므로 사용하지 않는다.
    ...
    // ExceptionHandler 에서 호출한다.
    // Validation 실패, 잘못된 요청, 서버 오류처럼 클라이언트에게 오류 응답이 반환된 횟수를 집계한다.
    //  HTTP 상태 코드별 자동 메트릭(http.server.requests)도 있지만,
    //  수업에서는 제한된 reason 태그를 직접 설계해보는 예제로 사용한다.
    public void recordApiError(String reason) {
        // API 오류는 주문 실패와 별도로 기록한다.
        // Validation 실패나 잘못된 조회처럼 주문 생성 흐름이 아닌 오류도 HTTP 사용자 경험에는 영향을 주기 때문이다.
    ...
    // popularProductQueryTimer 가 "얼마나 오래 걸렸는가"를 말해 준다면,
    // 이 Counter 는 "얼마나 자주 호출되는가"를 말해 준다.
    // 운영 우선순위는 지연 시간과 호출 빈도를 함께 보고 판단해야 한다.
    // G1) 지연 시간 많고, 호출 빈도 높은 것이 가장 우선순위
    public void recordPopularProductRequest() {
        popularProductRequestCounter.increment();
    }

    // 주문 생성 전체 시간 기록을 종료한다.
    // 현재는 결제 지연까지 포함하므로 shop.payment.duration 과 비교해 병목 비중을 설명할 수 있다.
