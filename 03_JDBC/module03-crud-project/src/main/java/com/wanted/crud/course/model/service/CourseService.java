...
public class CourseService {
    ...
    *  - 우리 프로젝트(회사) 규칙 상 어떤 순서로 어떤 처리를 하는 가를 다루는 구문을 의미한다.
    *  Service 계층이 해야할 일
    *  - 1. DAO 를 호출
    *  - 2. 데이터를 검증/가공
    *  - 3. 필요 시 여러 DAO 를 조합((EX- 강의를 삽입할 때 강의에 포합되는 챕터, 섹션 삽입)
    ...
    * */
    ...
    public boolean createCourseWithDefaultSection(CourseDTO newCourse, SectionDTO newSection) {
        ...
        /*
        * comment. courses 테이블에 강의를 insert
        *  course_sections 테이블에 섹션 insert
        * */

        try {
            /*
            * setAAutoCommit 을 끄는 이유는 2개의 작업 중 1개라도 오류가 나면
            ...
            * */
            ...
            // 이전에 만들어 두었던, 강의 등록 매서드 재활용
            // generatedCourseId = 강의 등록 시 생성된 PK 값이 들어있다.
             Long generatedCourseId = courseDAO.save(newCourse);

             if(generatedCourseId == null) {
                 throw new SQLException("강의 ID 생성에 실패했습니다.");
             }

            // newSection 객체의 courseId 는 null 이다.
            // 하지만, 위쪽에서 코스를 등록하며 발생한 PK 를 알았기 떄문에
            // 해당 courseId 를 newSection 객체의 courseId 필드에 넣을 것 이다.
            newSection.setCourseId(generatedCourseId);
             ...
             if(result == 0) {
                 throw new SQLException("섹션 생성에 실패했습니다.");
             }
