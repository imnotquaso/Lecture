...
public class CourseDAO {
    ...
    *  DAO 가 해야할 일
    *  - 1. SQL 구문을 실행한다.
    *  - 2. PreparesStatement 생성
    *  - 3. 전달받은 파라미터를 바인딩 한다.-> (? -> 값 대입)
    ...
    * */
    ...
    public Long save(CourseDTO newCourse) throws SQLException {
        ...
        try (PreparedStatement pstmt = connection.prepareStatement(query, PreparedStatement.RETURN_GENERATED_KEYS)) {
            ...
            // 결과 값은 정수 자료형, 즉 영향을 받은 행의 개수가 리턴된다.
    ...
    // course + section 으로 이루어진 데이터 반환
    public CourseSectionDTO findCourseWithSections(long course_Id) throws SQLException {
        ...
        try (PreparedStatement pstmt = connection.prepareStatement((query))) {
            ...
            while (rset.next()) {
                ...
                *  getLong(), getInt() 는 DB 에 null 을 그대로 담지 못하며
