...
public class ResponseEntityController {
    ...
    * */
    ...
    // user 전체 조회
    @GetMapping("/users")
    // 응답시 여러 명의 사람이 개발할 때, 공통 응답 템플릿을 만들어두는 것이 좋다.
    ...
    public ResponseEntity<ResponseMessage> findUserByNo(@PathVariable int userNo) {
        ...
        // 다만, JSON 타입이 아닌 데이터를 응답할 떄는 직접 조정할 필요가 있기 떄문에
        ...
        // return new ResponseEntity<>(responseMessage, headers, HttpStatus.OK); <- 같은 문장
    }

    // 신규 유저 등록하기
    @PostMapping("/users")
    // @RequestBody 는 JSON 타입의 데이터를 응답 받을 떄 사용한다.
    // 또한 Jackson 라이브러리가 기본적으로 동작하기 떄믄에
    ...
    public ResponseEntity<?> registUser(@RequestBody UserDTO newUser) {
        ...
        *  이럴 떄는 ResponseEntity<?> 와일드 카드를 정의해서 유연하게 대응하는 것이 좋다.
        *  1. 리소스를 반환하는 경우
        *  2. 리소스를 반환하지 않는 경우
        * */
