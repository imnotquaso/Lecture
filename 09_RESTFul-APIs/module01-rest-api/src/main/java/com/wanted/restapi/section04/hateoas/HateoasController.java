...
//}
    ...
    *  - withSelfRel : 현재 리소스 자체에 대한 'self' 링크를 축가
    *  - withRel : 관련된 다른 리소스에 대한 링크를 추가
    *  Hateoas 적용하게 되면 기존 2단계까지는 RestAPI 라고 불리우고 3단계까지 적용하게 되면 RestFulAPI 라고 부른다.
    *  Ful 이라는 것은 충분한 이라는 단어를 의미하며, 단순 API 를 호출할 때
    *  그 API 에 대한 정보 뿐만 아니라, 괸련된 리소스를 충분하게 제공하는 것을 의미한다.
    * */
