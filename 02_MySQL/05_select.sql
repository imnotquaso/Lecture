-- SELECT (조회)
-- SELECT SQL 전부다 대부분의 작업 90% 이상이 SELCET 조회 구문이다.
...
-- 개졸리다 진짜 개피고ㅗㄴ하다진짜ㅏ다아ㅏ아ㅏ아아아아아ㅏ아아ㅏ
...
-- users 테이블에서 user_id 가 10보다 큰 사용자 조회
select
	*
from users
where user_id > 10;
...
-- limit 
-- 조회 결과의 개수를 제한하는 기능이다.
-- 수백만 건의 데이터를 한번에 조회하게 되면, 시스템 상에 큰 부하를 주게 된다.
-- limit 는 '상위 5개만 보기', '1페이지에 10개 씩 보기' 등
-- 웹 어플리케이션의 페이징 기능을 구현하는데 필수 핵심 문법이다.

-- users 테이블에서 아이디, 이름, 가입일 조회할건데 가장 최근에 가입한 순서대로 나열
select
	user_id,
    name, 
    created_at
...
-- users 테이블에서 아이디, 이름, 가입일 조회할건데 가장 나중에 가입한 순서대로 나열
select
	user_id,
    name, 
    created_at
...
from users
where 
	(created_at BETWEEN '2024-04-01 00:00:00' and '2024-05-31 23:59:59')
	and 
    status = 'active'
    and
