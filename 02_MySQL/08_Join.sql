...
-- 테이블 정보는 각각의 테이블에 저장이 되어 있다.
-- 테이블을 나누는 것은 정규화라고 하며, 정규화에 의해
-- 비슷한 종류의 데이터끼리 테이블을 구성하게 된다. 

-- join 은 이렇게 분리 된 테이블 중 공통으로 포함하는 값(주로 PK-FK)
...
-- courses 테이블, users 테이블을 조합하여 과정 제목과, 그 과정을 만든 교육자
-- 이름을 조회하라.
...
-- 테이블 3개 조인하기
-- enrollments 테이블 기준으로 users, coursrs 를 연결하여
-- 어떤 학생이 어떤 과정에 등록했는지 학생 이름과 과정 제목, 등록일을 조회하라.
...
-- left join 과 where 절을 사용해서
-- 과정을 단 한번도 개설하지 않은 사용자를 조회하라
-- 조회 시 이름 조회
...
-- 모든 사용자 이름과, 연동된 소설 로그인 제공자(provider) 를 조회한다. 
-- (소설 로그인을 하지 않은 사용자는 null)
...
-- forum_posts 테이블을 사용해서 self_join 진행.
-- 대댓글, 댓글 내용을 함께 조회
