...
select
	email,
    replace(email, 'example.com', 'wnated.co.kr') as '변경 후'
from users;
...
-- 저런 포맷 형식 실제 정규 표현식에 기반한다. 정규 표현식은 절대 외우지 말고
-- 우리가 날짜를 변환해야 할 때 정규 표현식을 쓰면 어떨까? 라는 생각만 가지면 된다.
...
-- courses 테이블에 status 값에 따라 '게시됨', '초안' 으로 알아보기 쉽게 조회
...
-- grades 테이블의 점수에 따라서 95점 = A+, 90 = A, 85 = B+, 80 = B, 70 = C, 그 이하 = D
-- 예상 결과 : 조평훈 97 A+
-- 힌트 : users 테이블과 grades 테이블 join, 별칭으로 각 컬럼 밑 테이블 구분
