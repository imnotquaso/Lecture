...
-- LMS 마케팅팀에게는 데이터 분석팀이 사용자의 전체 정보를
-- 오픈하지 않고, 실제로 사용 될 정보인 이메일/이름만 전달하고자 한다.  

-- View 를 사용하는 이유
-- 편의성 : 복잡한 쿼리를 뷰로 만들게 되면, 이후에는 마치
-- 하나의 간단한 테이블처럼 쉽게 데이터를 조회할 수 있다.

-- 보안성 : 원본 테이블의 민감한 정보를 숨기고, 허용된 컬럼으로만
...
-- v_course_enrollments_stats 를 테이블처럼 사용해보기
select * from v_course_enrollments_stats;

-- View 를 사용해서 등록 학생 수가 3명 이상인 과정 찾아라.
select
	*
...
-- 뷰 삭제
-- deop view [view 명]
drop view v_course_endollments_stats;
