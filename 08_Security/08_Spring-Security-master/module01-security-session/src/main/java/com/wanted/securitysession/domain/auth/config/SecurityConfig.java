...
@Configuration
/* Spring Security 설정을 활성화하기 위해 반드시 필요한 어노테이션 */
@EnableWebSecurity
public class SecurityConfig {
    ...
    /*
     * - 세션이 생성될 때: HttpSessionEventPublisher 가 SessionRegistry 에 세션을 등록
     * - 세션이 소멸될 때: HttpSessionEventPublisher 가 SessionRegistry 에서 세션을 제거
     *
     * 이를 통해 SessionRegistry 는 현재 활성화된 세션 목록을 정확하게 유지할 수 있다.
     * */
    @Bean
    ...
    @Bean
    // security 설정에서 정적 리소스(css, js, img, video) 에 대한 요청을 제외하겠다는 설정이다. (static 파일 하위)
     ...
     * SecurityConfig 에서 직접 생성하여 순환 의존성 방지
     *
     * 로그인 실패 시
     * 1. 실패 횟수 증가
     * 2. 실패 로그 저장
    ...
    * comment.
    *  가장 중요한 Bean.
    *  Security 설정 시 해당 설정이 Main 이며, 여기에 사용자의 권한 별 접근 가능한
    *  URL, 로그인 페이지, ID, Password 등을 컨트롤 할 수 있으며 로그인 성공 시 어떤 동작을 할 지,
    *  실패 시 어떤 동작을 할 지 Customer Handler 도 등록할 수 있다.
    * */
    ...
    public SecurityFilterChain configure(HttpSecurity http,
        ...
        http.authorizeHttpRequests(auth -> { // 서버의 리소스에 접근 가능한 권한을 설정함
                    ...
                    // /admin 으로 시작하는 요청 url 은 ADMIN 권한을 가진 사람만 접근 가능
                    // ADMIN 권한을 가지지 않은 사람들은 403 에러 발생
                    auth.requestMatchers("/admin/*").hasAnyAuthority("ADMIN");
                    // /user 으로 시작하는 요청 url 은 USER 뿐만 아니라, ADMIN 도 접근 가능
                    auth.requestMatchers("/user/*").hasAnyAuthority("USER", "ADMIN");
                    // 그 외 요청들은 로그인을 완료하면 접근 가능
                    ...
                    // security 에서 username 은 우리가 흔히 아는 사용자 ID 생각하면 된다.
                    login.usernameParameter("user"); // 사용자 id 입력 필드 (input 의 name 과 일치)
                    login.passwordParameter("pass"); // 사용자 pass 입력 필드 (input 의 name 과 일치)
                    // Bean 으로 등록한 SuccessHandler 와 FailHHandler 사용
                    ...
                    // 사용자 1명 당 1개의 최대 세션을 통해 중복 로그인 등을 방지할 수 있다.
                    session.maximumSessions(1).sessionRegistry(sessionRegistry); // session 의 허용 개수를 제한
                 ...
                 * - Thymeleaf 의 _csrf 토큰을 폼에 포함하거나
