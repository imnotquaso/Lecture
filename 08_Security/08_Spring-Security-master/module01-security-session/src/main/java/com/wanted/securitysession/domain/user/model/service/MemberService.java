...
public class MemberService {
    ...
    public Integer regist(SignupDTO signupDTO) {
        ...
        if (userRepository.existsByUserId(signupDTO.getUserId())) {
            return null; // 중복된 아이디가 존재함을 null 로 표시
        }

        // 중복되는 Id 가 없으면 회원가입 실행

        try {
            // DTO 를 엔티티로 변환 (ModelMapper 사용 후 Builder 패턴으로 password 설정)
            User user = modelMapper.map(signupDTO, User.class)
                    // 비밀번호 등과 같은 민감한 정보는 Security Config 작성했던 Bcrypt 를 활용해서 암호화 처리를 할 것 이다.
    ...
    /**
     * 사용자의 id를 입력받아 DB 에서 회원을 조회하는 메서드
    ...
    public void resetLoginFailCount(String username) {
        ...
        // 로그인 성공을 하게 되면 1~4 사이로 실패를 하게 되면 성공시 0으로 초기화 해준다.
