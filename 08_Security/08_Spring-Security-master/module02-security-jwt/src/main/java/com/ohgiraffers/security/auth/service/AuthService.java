...
public class AuthService {
    ...
    /**
     * 🎯 로그인 처리: AccessToken 만 클라이언트로 응답,
     *    RefreshToken 은 서버(In-Memory)에 저장
     */
    @Transactional
    public TokenResponseDTO login(LoginRequestDto loginRequestDto) {
        // 1. 사용자 인증 시도
        // G1) authenticate() 에 사용자 정보가 들어있다고 판단
        // AuthenticationManager.authenticate()는 인증 실패 시 AuthenticationException 을 발생시킨다
        ...
        // 해당 사용자의 기존 Refresh Token 이 있다면 삭제 (선택적: 하나의 세션만 허용하는 경우)
