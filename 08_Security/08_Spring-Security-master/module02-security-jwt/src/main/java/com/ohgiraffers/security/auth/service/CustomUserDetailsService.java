...
- Spring Security 는 `SecurityContextHolder`에 인증 객체를 저장하고, 이 인증 객체에서 사용자 정보를 추출할 수 있음
...
public class CustomUserDetailsService implements UserDetailsService {
    ...
    *  username 을 바탕으로 DB 에서 실제로 존재하는 사용자인 지 검색한 후 존재하는 유저이면
    *  return 구문에서 SecurityContext 에 들어갈 사용자 객체를 만들어준다.
    * */

    @Override
    @Transactional
    // G1) security 에서 기본적으로 제공되는 userId
    //  매우 중요
    //  사용자 아이디를 바탕으로, securityContext 에 로그인 정보를 넣어줌
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        ...
        // G1) return 이 어디로 들어가는지가 매우 중요
        //  security context 로 들어가게 됨
    ...
    private Collection<? extends GrantedAuthority> mapRolesToAuthorities(List<Role> roles) {
        ...
        // 각 Role Enum 의 이름(예: "ROLE_USER")을 사용하여 SimpleGrantedAuthority 객체를 생성
        // Role Enum 의 name() 메소드가 "ROLE_" 접두사를 포함한 문자열을 반환한다고 가정
        return roles.stream()
