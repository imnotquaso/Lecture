...
public class SecurityConfig {
    ...
    * */
    ...
    private final JwtAuthenticationFilter jwtAuthenticationFilter; // g1) jwt. 작성
    private final CustomAccessDeniedHandler accessDeniedHandler; // g1) handler. 작성
    private final CustomAuthenticationEntryPoint authenticationEntryPoint;// g1) handler. 작성
    ...
    public CorsConfigurationSource corsConfigurationSource() {
        ...
        configuration.setAllowedHeaders(Arrays.asList(
                "Authorization", // G1) <- header key 값에 token 넣어서 다름
                "Content-Type",
                "X-Requested-With",
                "Accept",
                "Origin",
        ...
        // true 로 설정해야 쿠키를 사용한 인증이나 Authorization 헤더를 통한 토큰 인증이 가능.
     ...
     * Spring Security 에서 AuthenticationManager 는 인증(로그인) 과정을 총괄하는 중요한 클래스이다.
     ...
     * AuthenticationManager 를 가져오는 방식다.
...
-     *
    ...
    // G1) passwordEncoder 가 interface 여서 bean 으로 등록?
    @Bean
    public BCryptPasswordEncoder passwordEncoder() {
        return new BCryptPasswordEncoder();
    }

    /* 인증/인가 관련 Security 에 가장 중요한 설정 Bean */
    @Bean
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        ...
        * */
        return http
                ...
                /*
                * addFilterBefore
                * HttpSecurity 설정 내에서 사용되며, Spring Security 의 기존 필터 체인에 사용자 정의 필터를 특정 필터 앞에 추가할 때 사용
                * */
                // G1) security filter 작동 전 JWT filter 를 동작하게 함
                .addFilterBefore(jwtAuthenticationFilter, UsernamePasswordAuthenticationFilter.class)
                /*
                * exceptionHandling
                * 설정 내에서 Spring Security 가 보안 관련 예외를 처리하는 방식을 지정
                * */
