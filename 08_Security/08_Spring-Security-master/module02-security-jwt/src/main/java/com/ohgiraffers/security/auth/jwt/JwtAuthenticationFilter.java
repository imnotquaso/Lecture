...
/*
 JwtAuthenticationFilter 는 사용자의 요청 헤더에서 JWT 를 추출하여,
 해당 토큰이 유효하다면 사용자 정보를 기반으로 Authentication 객체를 생성하고
 SecurityContext 에 등록해 Spring Security 인증 체인에 통합하는 역할을 한다.
...
*/
@Component
// G1) 요청 시 한번만 동작하는 필터 생성 (JWT -> Security Filter)
        ...
        try {
            ...
            try {
                ...
                if (refreshTokenValue != null) {
                    ...
                    // 자동 재발급 시도했으나, 요청에 Refresh Token 이 없는 경우
            ...
            // Access Token 이 만료 외의 이유로 유효하지 않은 경우 (예: 서명 오류, 잘못된 형식)
            SecurityContextHolder.clearContext();
        }
