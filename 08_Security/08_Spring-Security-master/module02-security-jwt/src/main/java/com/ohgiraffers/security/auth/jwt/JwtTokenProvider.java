...
/*******************************************
 📖 개념: JwtTokenProvider 의 역할과 구조
 ********************************************/

/*
 JwtTokenProvider 는 JWT 인증 시스템의 핵심 유틸리티로,
 ...
 ✅ 설계 이유
 - JwtTokenProvider 는 인증 필터나 컨트롤러에서 토큰 처리 로직을 분리시켜
   SRP(단일 책임 원칙)를 지키고 테스트/유지보수성을 향상시킨다.

 ✅ 실무 고려사항
 - 서명 키는 application.yml 이 아닌 환경 변수나 Secret Manager 로 외부화 필요
 - RefreshToken 의 경우 DB 또는 Redis 와 연동하여 관리하는 것이 일반적
*/
@Component
public class JwtTokenProvider {

    // JWT 토큰을 만들어 제공하는 클래스
    ...
    /*
     * key
     * - JWT 의 서명(Signature)을 생성하고 검증하는 데 사용되는 핵심 암호화 키 객체로
     *   사용자가 입력한 평문을 암호화 알고리즘에 맞춰 byte[]로 저장하게 된다.
     * - 이 `key`의 기밀성 유지는 JWT 기반 인증 시스템의 보안에서 가장 중요한데
     * - 특히 HMAC 과 같은 대칭키 알고리즘에서 이 키가 외부에 노출될 경우, 공격자는
    ...
    public String createAccessToken(Authentication authentication) {
        ...
        /*
        * comment.
        *  Jwts.builder 로 만들어지는 정보가 실제 Front 에서 활용할 수 있는 로그인 유저 관련 정보다.
        *  현재는 userId 와 권한이 들어있어서 Front 에서는 이 2개의 값을 바탕으로 활용할 수 있게 된다.
        * */
        // G1) 가장 중요한 부분
        return Jwts.builder()
                // G1
                .subject(username)
//                .subject("추가적인 값") G1) 탈취당해도 괜찮은 값, 권한, unique 나 Pk 값을 주로 넣음
                .claim("roles", authorities)
                .issuedAt(new Date())
                .expiration(new Date(System.currentTimeMillis() + ACCESS_TOKEN_EXPIRE_TIME))
                .signWith(key,  Jwts.SIG.HS512) // ✅ signWith 만 전달하면 HS256 자동 적용
                .compact();
    }

    // 🎯 RefreshToken 생성 // G1) DB 내의 실제로 노출되지 않는 토큰
    ...
    public String getUsernameFromToken(String token) throws InvalidJwtCustomException {
        ...
        } catch (JwtException e) { // extractClaims 가 InvalidJwtCustomException 을 던지지만, 더 넓게 잡을 수 있음
...
/*
 * - 토큰 재사용 방지를 위해 Redis 에 RefreshToken 저장 및 블랙리스트 처리 전략 필요
 * - key 는 `@PostConstruct`에서 디코딩/변환 → `@Value`만 사용할 경우 Spring Context 순서에 따라 NullPointer 발생 가능
 * */
