...
/*
✅ 왜 필요한가?
- AccessToken 은 일반적으로 짧은 수명(15분~1시간)을 가짐 → 유출 시 피해 최소화
- 사용자가 계속 로그인 상태를 유지하려면, 만료된 AccessToken 을 RefreshToken 으로 갱신해야 함
- RefreshToken 은 상대적으로 긴 수명을 가지므로 반드시 안전하게 저장되어야 함

✅ 언제 사용되는가?
- 사용자가 AccessToken 이 만료된 상태에서 자동 로그인하거나
- 클라이언트가 백그라운드에서 갱신 요청을 보내는 경우

✅ 실무 고려사항
- RefreshToken 을 DB 또는 Redis 에 저장하여 상태 관리 (탈취/만료 판단)
...
4. 클라이언트가 액세스 토큰 재발급을 요청하며 리프레시 토큰을 보내오면, 서버는 DB 에서 해당 토큰을 조회
...
/*
🚀 심화
- 대규모 서비스에서는 RefreshToken 을 DB 보다 Redis 에 저장하여 속도 향상
  - Redis: TTL 기능으로 자동 만료 관리 가능
  - Key 구성 예: `refresh:userId` → value: token

- 보안 강화를 위한 확장 전략:
  - User-Agent, IP, deviceId 등 추가 메타데이터와 함께 저장
  - 같은 유저라도 다른 디바이스에선 다른 RefreshToken 을 발급

- 토큰 블랙리스트 정책을 Redis Set 으로 구현하여 재사용 방지 가능
* */
