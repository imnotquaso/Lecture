...
public class RequestController {
    ...
    * */
    ...
    public String registMenu(Model model, WebRequest request) {
        ...
        // 1. 데이터 꺼내기(request.getParameter 이용)
        String menuName = request.getParameter("name");
        int menuPrice = Integer.parseInt(request.getParameter("price"));
        int categoryCode = Integer.parseInt(request.getParameter("categoryCode"));

        // 2. 비지니스 로직 만들기
        String message = menuName + "을(를) 신규 메뉴 목록 " + categoryCode + "번 카테고리에 " + menuPrice + " 원 으로 등록 성공했씁니다.";

        // 3. 결과 화면에 전달할 데이터 담기
        model.addAttribute("message", message);

        // 4. 어떤 HTML 화면을 보여줄지 결정
    ...
    *  주의 view 의 name 속성과 일치하게 작성해야 400 error 발생이 안되며,
    *  만약 name 속성과 일치하고 싶지 않으면 이름을 명시해야 한다.
    * */

    @PostMapping("modify")
    public String mofidyMenu(Model model, @RequestParam("modifyName") String name,
    ...
    *  Model 객체에 응답시 addAttribute 를 하지 않아도, 네이밍 규칙에 의해 사용할 수 있다.
    ...
    *  @SessionAttributes -> 클래스 레벨에 작성한다.
    *  session 에 담을 Key 값을 설정해두면, Model 영역에 해당하는 Key 로 값이 추가되는 경우 자동으로 session 에 등록해준다.
    * */
    ...
    /*
    * comment.
    *  SessionAttribute 방식은 Servlet 에서 Session 을 만료시키는 invalidate() 메소드로는 할 수 없다.
...
/*
* comment.
*  HTML-<form action="..."> -> Controller 의 @RequestMapping 주소
*  HTML-<form method="..."> -> Controller 의 @GetMapping/@PostMapping
*  HTML-<input name="id"> -> Controller 의 @RequestParam("id)
*  Java-model.addAttribute("A", B) -> 결과 HTML 의 ${A}
*  Java-return"request/loginResult"-> templates/request/loginResult.html
* */