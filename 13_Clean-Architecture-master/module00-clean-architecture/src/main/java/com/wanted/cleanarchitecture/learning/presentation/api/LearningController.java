...
public class LearningController {
    ...
    @Operation(
            summary = "모듈 완료 처리",
            description = "사용자가 특정 모듈을 완료 처리한다. 활성 수강 여부는 LearningAccessPolicy 에서 검증한다."
    )
    public ResponseEntity<ApiResponse<Void>> completeModule(
            @Parameter(description = "완료 처리할 모듈 ID", example = "1")
            @PathVariable Long moduleId, // G1) URL 에서 moduleId 추출
            @Valid @RequestBody CompleteModuleRequest request // G1) JSON 바디 받음
    ) {
        // G1) command 변환,
