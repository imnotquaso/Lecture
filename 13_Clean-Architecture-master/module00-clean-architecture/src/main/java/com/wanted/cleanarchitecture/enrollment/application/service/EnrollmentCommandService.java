 ...
 *
 * 이 Service 가 catalog 의 Course 를 직접 조회하지 않는 이유:
 * - enrollment 컨텍스트는 Course 전체 구조를 알 필요가 없다.
 * - 수강 신청에 필요한 것은 "강의가 공개되었는가?"라는 사실뿐이다.
 * - 그 사실은 EnrollmentEligibilityPolicy 가 CourseCatalogPort 를 통해 확인한다.
 */
@Service
@Transactional
// G1) EnrollmentCommandUseCase 를 상속하는 EnrollmentCommandService 가 호출
