...
/*
 * EnrollmentRepositoryAdapter 는 EnrollmentRepository port 의 JPA 구현체다.
 * 이 클래스의 핵심 책임은 domain model 과 JPA entity 사이의 변환이며, 비즈니스 규칙을 판단하지는 않는다.
 */
@Repository
@Transactional
public class EnrollmentRepositoryAdapter implements EnrollmentRepository {
    ...
    public Enrollment save(Enrollment enrollment) {
        // 새 Aggregate 와 기존 Aggregate 저장 흐름을 통일해 application 에는 같은 port 만 노출한다.
    ...
    private Enrollment toDomain(EnrollmentJpaEntity entity) {
        // 저장 모델을 순수 domain model 로 바꿔야 상위 계층이 JPA 에 오염되지 않는다.
