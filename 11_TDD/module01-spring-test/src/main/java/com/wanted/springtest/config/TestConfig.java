...
public class TestConfig {
     ...
     * 'test' 프로파일이 활성화될 때만 이 Bean 이 등록된다.
     *
     * 이는 테스트 환경에서 특별한 동작이 필요한 경우에 활용된다.
     * 예를 들어, 외부 API 호출을 Mock 으로 대체하거나,
    ...
    public CalculatorService testCalculatorService() {
        ...
        /* 설명.
         *  CalculatorService 를 상속받은 익명 클래스를 생성한다.
    ...
    /**
     * 테스트 환경에서 사용할 설정 정보를 담는 Bean 이다.
     *
     * 이 Bean 은 테스트에서 환경별 설정값을 확인하는 데 사용된다.
     * 예를 들어, 테스트 환경에서는 다른 타임아웃 값이나
     * 다른 URL 을 사용해야 하는 경우에 활용한다.
     *
     * @return 테스트 설정 정보
     */
    @Bean
     ...
     *
     * 실제 프로젝트에서는 @ConfigurationProperties 를 사용하여
     * application.yaml 에서 값을 읽어오는 것이 일반적이다.
     */
