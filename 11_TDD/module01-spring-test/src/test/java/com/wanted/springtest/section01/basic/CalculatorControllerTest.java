...
public class CalculatorControllerTest {
    ...
    /*
    * comment.
    *  MockMvc 는 실제 Tomcat 서버를 띄우지 않고, HTTP 요청/응답을 시뮬레이션 할 수 있다.
    *  -> Application 을 실행하지 않아도 가능하다는 의미
    ...
    * comment.
    *  @MockitoBean : 가짜 객체
    *  - 실제 IoC 컨테이너에 등록된 Service 를 사용하는 것이 아닌 Mock(가짜) 객쳉를 주입 받는다.
    ...
    * */
    ...
    void 더하기_API_올바른_결과_리턴_테스트() throws Exception {
        ...
        mockMvc.perform(
                ...
                .andDo(print()) // 요청 밒 응답 정보를 콘솔에 출력
                .andExpect(status().isOk()) // HTTP 응답 200 을 기대
                // $ = JSON 데이터의 최상위(ROOT) 객체 (=object)
    ...
    /*
    * comment.
    *  TestCode 는 정상 동작을 하는 것을 테스트 하는 것 보다는 예외(잘못된 값) 을 테스트 하는 것이 더 중요하다.
    * */
    @Test
    @DisplayName("잘못 된 파라미터 타입 전송 시 Bad Request 를 반환한다.")
