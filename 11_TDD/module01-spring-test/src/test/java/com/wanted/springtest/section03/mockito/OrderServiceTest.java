...
*  - Mockito 는 Spring Context 를 전체 로드하는 것이 아닌
*  - 필요한 Bean 만 Mock(가짜) 로 생성해서 불필요한 리소스 낭비를 방지한다.
* */
...
public class OrderServiceTest {
    ...
    * */
    ...
    void 결제_성공_시_주문_처리_성공_테스트() {
        ...
        *  PaymentService 는 부가적인 요소이며, OrderService 에 영향을 주면 안된다.
        *  - when().thenReturn()
    ...
    void 결제_성공_시_이메일_발송_테스트() {
        ...
        // then : 이메일 발송 메서드가 호출되었는 지 검증
        // 결제 -> 주문 -> 이메일
        // verify() : 메서드 호출 여부를 검증
        // ArgumentMatchers : 파라미터 매칭을 확인하는 도구
        // anyString() : 어떤 문자열이든 상관없이 호출 여부 확인
        // eq : 특정 값과 정확히 일치하는지 확인
    ...
    void 결제_실패_시_이메일_발송_테스트() {
        ...
        // then : 이메일 발송 메서드가 호출되지 않았는 지 검증
        // never() : 한번도 호출되지 않았음을 검증
